...
vector<int> pi( string s , int m ) {
   vector<int> p(m, 0);
   for ( int i = 1 ; i < m ; i ++) {
      int j = p[i - 1];
      while ( j > 0 and s.at(i) != s.at(j)) j = p[j - 1];
      p[i] = j += s.at(i) == s.at(j);
   }
...
vector<int>  kmp ( string s , string pat ) {
   vector<int> occ(n , 0 );
   vector<int> lps = pi ( pat , m);

   int i = 0 , j = 0 ;
   while ( i < n ) {
      if ( s.at(i) == pat.at(j)) i ++ , j ++;
      if ( j == m) occ[i - j] = 1 , j = lps[j - 1];
...
vector<int> z ( string s ) {
   vector<int> zarr ( s.size() + 1 , 0 );
   for ( int i = 1 , l = 0 , r = 0 ; i < sz(s) ; i ++) {
      if ( i <= r ) zarr[i] = min ( r - i + 1 , zarr[i - l]);
      while ( i + zarr.at(i) < n and s.at(zarr[i]) == s.at(i + zarr[i]))
         ++zarr[i];
      if ( i + zarr.at(i) - 1 > r) l = i , r = i + zarr.at(i) - 1;
   }
